...
class metaRL_ensemble():
    ...
    logger_kwargs = None,use_latent = True):
        ...
        # B optimizer for B models
        lists = []
        for i in range(self.ensemble_size):
            lists = lists + [*encoder.models[i].parameters()]
        lists += [*self.decoder.reward_net.parameters()]
        lists += [*self.decoder.state_net.parameters()]
        self.VAE_optimizer = Adam(lists, lr=CFG.lr_VAE)
    ...
    def update_VAE(self, data,epoch_):
        ...
        for update_epoch in range(self.CFG.VAE_update_epoch):
            ...
            self.encoder.set_hidden_shape(self.CFG.lstm_layers, self.CFG.batch_size, self.CFG.lstm_hidden_dim)
            for i in range(self.CFG.construct_step):
                ...
                (mu_news, logvar_news, latent, _) = self.encoder.forward(self.tensor(o),self.tensor(a),self.tensor(r))
                if (i):
                    val = None
                    for member in range(self.ensemble_size):
                        if (member):
                            val += self.compute_kl_loss(mus[member],mu_news[member],logvars[member],logvar_news[member]).mean()
                        else:
                            val = self.compute_kl_loss(mus[member],mu_news[member],logvars[member],logvar_news[member]).mean()
                    val /= self.ensemble_size
                    if (KL_loss):
                        KL_loss += val
                    else:
                        KL_loss = val
            ...
            for i in range(self.CFG.construct_step + self.CFG.inference_step-1):
                o_his,next_o,r,a = o_list[start_idx + i],next_o_list[start_idx + i],r_list[start_idx + i],a_list[start_idx + i]
                if (o_expect is None):
                    o = o_his
                else:
    ...
    def train(self):
        ...
        for epoch in range(self.CFG.epochs):
            ...
            self.encoder.set_hidden_shape(self.CFG.lstm_layers, 1, self.CFG.lstm_hidden_dim)
            ...
            for t in range(self.policy.local_steps_per_epoch):
                ...
                # Update obs (critical!)
                if (self.use_latent):
                    (mu,logvar,new_latent,hidden) = self.encoder.forward(self.tensor([o]),self.tensor(a),self.tensor([[r]]))
                    latent_diff.append(np.sum((latent-new_latent).pow(2).detach().numpy()))
                    latent = new_latent
                    mu_ls = np.concatenate(((mu_ls,mu.detach().numpy().reshape(-1))))
                    std_ls.append(np.mean(np.exp(0.5 * logvar.detach().numpy())))
                o = next_o
    ...
    def test(self):
        for ttest in range(10):
            ...
            while( not done):
                ...
                rewards.append(r)
                if (self.use_latent):
                    (mu,logvar,latent,hidden) = self.encoder(self.tensor([o]),self.tensor(a),self.tensor([[r]]))
    ...
    def save_model(self):
        ...
        if (self.encoder):
            for i in range(self.ensemble_size):
                torch.save(self.encoder.models[i].state_dict(), osp.join(fpath,f'encoder{i}.pt'))
        if (self.decoder):
    ...
    def load_model(self,fpath):
        ...
        for i in range(self.ensemble_size):
            self.encoder.models[i].load_state_dict(torch.load(osp.join(fpath,f'encoder{i}.pt')))
